...
//     return a.GPA > b.GPA;
// }
bool findsv(const sinhvien &sv, const string &keyword)
{
    // Chuyển đổi tất cả các ký tự của tên sinh viên và từ khóa thành chữ thường để so sánh
    string lowerHoTen = sv.name;
    string lowerKeyword = keyword;
    transform(lowerHoTen.begin(), lowerHoTen.end(), lowerHoTen.begin(), ::tolower);
    transform(lowerKeyword.begin(), lowerKeyword.end(), lowerKeyword.begin(), ::tolower);

    // Tìm kiếm từ khóa trong tên sinh viên
    if (lowerHoTen.find(lowerKeyword) != string::npos)
    {
        return true;
    }
    return false;
